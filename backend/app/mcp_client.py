"""Minimal, loopback-only MCP client used by local source connectors.

The application deliberately supports only the Streamable HTTP transport here.
TAPD credentials remain owned by the local MCP server and never cross this API.
"""
...
def validate_local_mcp_url(url: str) -> str:
    ...
    if (parsed.hostname or "").lower() not in LOOPBACK_HOSTS:
        raise McpError("MCP 仅允许连接本机回环地址（localhost、127.0.0.1 或 ::1）")
...
class StreamableHttpMcpClient:
    """Small synchronous JSON-RPC client for a running local MCP server."""
    ...
    def __init__(
        ...
        *,
        ...
        self.endpoint_url = validate_local_mcp_url(endpoint_url)
    ...
    def _post(self, payload: dict[str, Any], *, notification: bool = False) -> dict[str, Any]:
        ...
        try:
            ...
            raise McpError(f"无法连接本地 MCP Server：{exc}") from exc
...
def inspect_server(endpoint_url: str, *, timeout: float = 0.45) -> dict[str, Any]:
    ...
    endpoint_url = validate_local_mcp_url(endpoint_url)
...
def _config_candidates() -> tuple[list[tuple[str, str]], list[str]]:
    """Read URL-only MCP metadata from common local configs without exposing secrets."""
    ...
    http_servers: list[tuple[str, str]] = []
        ...
        for mapping in mappings:
            for name, config in mapping.items():
                ...
                if isinstance(url, str):
                    try:
                        http_servers.append((str(name), validate_local_mcp_url(url)))
                    except McpError:
                        continue
                elif config.get("command"):
                    stdio_names.append(str(name))
...
def discover_local_servers(manual_url: str = "") -> list[dict[str, Any]]:
    ...
    candidates: list[tuple[str, str]] = configured
    if manual_url:
        candidates.insert(0, ("手工地址", validate_local_mcp_url(manual_url)))
    env_urls = [
        item.strip()
    ...
    for url in (*env_urls, *DEFAULT_DISCOVERY_URLS):
        try:
            candidates.append(("自动发现", validate_local_mcp_url(url)))
        except McpError:
            continue
    unique: dict[str, str] = {}
    ...
    def probe(item: tuple[str, str]) -> dict[str, Any] | None:
        url, name = item
        try:
            parsed = urlparse(url)
            port = parsed.port or (443 if parsed.scheme == "https" else 80)
            with socket.create_connection((parsed.hostname or "127.0.0.1", port), timeout=0.15):
                pass
            return {"name": name, **inspect_server(url)}
        except (McpError, OSError) as exc:
            if manual_url and url == validate_local_mcp_url(manual_url):
                return {
                    "name": name,
                    "endpoint_url": url,
                    "transport": "streamable_http",
